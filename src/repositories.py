...
def get_airplane_data_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(airplanes_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_airports_data_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(airports_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_boarding_passes_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(boarding_passes_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_bookings_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(bookings_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_flights_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(flights_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_passengers_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(passengers_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_routes_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(routes_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_segments_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(segments_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_seats_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(seats_query, {"limit": limit, "offset": offset})
        return result.mappings().all()


def get_tickets_repository(limit: int = 50, offset: int = 0):
    with get_db_context() as session:
        result = session.execute(tickets_query, {"limit": limit, "offset": offset})
        return result.mappings().all()
